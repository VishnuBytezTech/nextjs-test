...
// for fetch data from redux
import { useEffect } from "react";
import { getPermissions } from "@/api/permissions/permissionApi";
import { useDispatch, UseDispatch, UseSelector } from "react-redux";

import { useSelector } from "react-redux";
import { RootState } from "@/redux/store";
import { fetchPermissions } from "@/redux/features/permission-slice";
...
export default function PermissionListTable() {
  ...
  // api calling and add teh values on redux

  const dispatch = useDispatch();
  const permissions = useSelector(
    (state: RootState) => state.permissionReducer.permissions
  );
  ...
  //       setListData(permissions);
  //     }
  //   } , [permissions]);

  // useEffect(() => {
  //   if (permissions.length > 0 && listData !== permissions) {
  //     setListData(permissions);
  //   }
  ...
  // Transform permissions from Redux store to match the table data format
  ...
  console.log("teh permission data :::", permissionData);
  ...
  const {
    ...
    setSelectedRowKeys,
    selectedRowKeys,
  } = useTable(permissionData, pageSize, filterState);
