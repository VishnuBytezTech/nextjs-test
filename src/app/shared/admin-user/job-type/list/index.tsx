...
import { GetColumns } from '@/app/shared/admin-user/job-type/list/columns';
...
export default function JobTypeListTable() {
  ...
  const next = useSelector((state:RootState) => state.jobtypereducer.next)
  const prev = useSelector((state:RootState) => state.jobtypereducer.previous)
  ...
  useEffect(()=>{
    dispatch(fetchJobTypesThunk({authToken: authToken as string, limit, offset}))
  ...
  const {
    ...
    setSelectedRowKeys,
    selectedRowKeys,
  } = useTable(jobTypeData, pageSize, filterState);
  ...
  return (
    <>
      {
        jobTypeData && jobTypeData.length ?  (
