...
import { GetColumns } from '@/app/shared/admin-user/commodity/list/columns';
...
import { fetchCommoditiesThunk } from '@/redux/features/commodity-slice';
...
export default function CommodityListTable() {
  ...
  const next = useSelector((state:RootState) => state.commodityreducer.next)
  const prev = useSelector((state:RootState) => state.commodityreducer.prev)

  useEffect(()=>{
    dispatch(fetchCommoditiesThunk({authToken: authToken as string, limit, offset}))
  }, [ dispatch, authToken, offset, limit ])
  ...
  const {
    ...
    setSelectedRowKeys,
    selectedRowKeys,
  } = useTable(commodityData, pageSize, filterState);
